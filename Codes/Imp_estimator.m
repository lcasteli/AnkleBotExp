%% Computing Impedance and Frequency
% Run as Imp_estimator('SubjectName')
% Replace "SubjectName" with the subject name of the data you will use
% @author Mo Rastgaar, Chen Jia, Houman Dallali, Leslie
%         Department of MEEM, Michigan Technological University
% last updated: 1/29/2016

function [ ] = Imp_estimator( SubjectName)
flimit=100;

for testNumber=1:15;

% load opendata\SubjectNamecl
if ~exist('opendata')
    error('Cant find opendata, go to [ankleYawExperiments\Code] folder')
end


if sum(strcmp(SubjectName,{'LES','HN','EVD','EML','GI'}))
    load opendata\male
else
    load opendata\female
end

% abot=open16;   G_s=0; K_s=10;   % anklebot alone (with shoe)

abot=calibration;    G_s=0; K_s=10; %anklebot alone (with shoe & no subject)

human=load(['opendata\' SubjectName '\Imp' num2str(testNumber)]);
human=human.rawImpedance; G_a=0; K_a=10; % with human body,
...
plot(x_axis,phase,str3,'LineWidth',2);
...
coherence=real((abs(P_aa_2_7.*P_aa_2_8-P_aa_2_9.*P_aa_2_10).^2)./(P_aa_2_11.*P_aa_2_11.*P_aa_2_12.*P_aa_2_13)./(1.-C_aa_2_14)./(1.-C_aa_2_15));
figure(n+1);subplot(3,1,3,'YTick',[0 0.2 0.4 0.6 0.8 1],'YMinorTick','on','XTick',[1 10 lim_x],'XScale','log','fontsize',font);hold on;
plot(x_axis,coherence,str3,'LineWidth',2);
ylabel('Coherence');xlabel('Hz');grid on;box on;xlim([0.7 lim_x]);ylim([0 1]);legend('co-contraction','passive')

%% save all results
if ~exist(['ImpFreqdata\' SubjectName ])
    mkdir(['ImpFreqdata\' SubjectName ])
end

save(['ImpFreqdata\' SubjectName '\' num2str(testNumber)],'impedance','x_axis');  % You may change the directory here 
